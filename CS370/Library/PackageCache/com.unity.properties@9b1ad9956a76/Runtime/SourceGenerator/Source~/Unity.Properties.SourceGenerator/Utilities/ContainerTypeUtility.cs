...
namespace Unity.Properties.SourceGenerator
{
    ...
    static class ContainerTypeUtility
    {
        ...
        public static IEnumerable<ITypeSymbol> GetPropertyContainerTypes(IAssemblySymbol assembly, CancellationToken cancellationToken)
        {
        ...
        static IEnumerable<ITypeSymbol> GetTopLevelPropertyContainerTypes(IAssemblySymbol assembly)
        {
            ...
            var generatePropertyBagsForTypesQualifiedWithAttributes = assembly.Modules.First().ReferencedAssemblySymbols
                ...
                .Where(x => x.AttributeClass.Name == "GeneratePropertyBagsForTypesQualifiedWithAttribute")
                .Where(x => x.ConstructorArguments.Length > 0)
                .ToImmutableHashSet();
        ...
        static IEnumerable<ITypeSymbol> GetPropertyContainerTypesRecursive(ITypeSymbol symbol, ISet<string> visited)
        {
            if (!IsContainerType(symbol) || !visited.Add(symbol.GetGlobalSanitizedName())) yield break;
        ...
        static bool MatchesAnyQualifiedWithAttribute(ITypeSymbol symbol, ImmutableHashSet<AttributeData> attributes)
        {
            foreach (var i in attributes)
            {
                ...
                var options = null != i.ConstructorArguments[1].Value ? (TypeGenerationOptions) i.ConstructorArguments[1].Value : (TypeGenerationOptions) 0;

                if (MatchesTypeOptions(symbol, options) && symbol.AllInterfaces.Contains(type, SymbolEqualityComparer.Default))
                    return true;
            }
        ...
        static bool MatchesTypeOptions(ITypeSymbol type, TypeGenerationOptions options)
        {
            if (type.IsValueType) return (options & TypeGenerationOptions.ValueType) != 0;
            return (options & TypeGenerationOptions.ReferenceType) != 0;
        }
        ...
        static bool IsContainerType(ITypeSymbol symbol)
        {
            ...
            // Static types are not supported yet.
